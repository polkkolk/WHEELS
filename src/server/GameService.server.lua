...
KillCamRespawnEvent.OnServerEvent:Connect(function(player, action)
	if action == "respawn" or action == "lobby" then
		if action == "lobby" then
			player:SetAttribute("_ForceLobby", true)
		else
			player:SetAttribute("_ForceLobby", nil)
		end
...
local function runRound(mapCfg, modeCfg)
	...
	local function hookRespawn(p)
		local conn = p.CharacterAdded:Connect(function(char)
			...
			if p:GetAttribute("_ForceLobby") then return end
