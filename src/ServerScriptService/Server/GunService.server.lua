...
GunFireEvent.OnServerEvent:Connect(function(player, weaponName, origin, direction, target, hitPosition)
    ...
    local tween = ts:Create(tracer, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    if target and target.Parent then
        ...
        if humanoid and humanoid.Health > 0 and not humanoid:GetAttribute("IsDead") then
            ...
            if wasAlive and humanoid.Health <= 0 then
                ...
                -- Only count toward stats/round if victim is a real player
                local victimPlayer = Players:GetPlayerFromCharacter(hitModel)
                if victimPlayer then
...
local winsLeaderboardStore  = DataStoreService:GetOrderedDataStore("WinsLeaderboard")
local killsLeaderboardStore = DataStoreService:GetOrderedDataStore("KillsLeaderboard")
...
local function saveStats(player)
    ...
    if wins then
        ...
        pcall(function() winsLeaderboardStore:SetAsync(player.Name, wins.Value) end)
    end
    if money then
...
Players.PlayerAdded:Connect(function(player)
    ...
    lifetimeCoins.Parent = hiddenStats
    ...
    local okC, savedCoins = pcall(function()
        return statsStore:GetAsync("Coins_" .. player.UserId)
    end)
    if okC and savedCoins then money.Value = savedCoins else money.Value = 0 end
    
    local okLC, savedLifetime = pcall(function()
