...
local function applyIndicator(name, model)
	...
	-- FIX: Must WaitForChild because the client spots the Model before the server finishes replicating its children over the network!
	local hrp = model.PrimaryPart or model:WaitForChild("HumanoidRootPart", 3)
	local billboard
	if hrp then
		...
		billboard.LightInfluence        = 0                     -- DO NOT SHADOW THIS TAG
		...
		lbl.Text                   = name
		lbl.TextColor3             = Color3.fromRGB(0, 255, 100) -- Always Bright Green for teammates!
		lbl.Font                   = Enum.Font.GothamBlack
		lbl.TextSize               = 28
		lbl.TextStrokeTransparency = 0.2
		lbl.TextStrokeColor3       = Color3.fromRGB(0, 50, 0)
