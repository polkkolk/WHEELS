...
local function onBloodEvent(pos, normal, victim, attacker, bloodType)
	...
	local function spawnBlob(centerPos, finalSize, overrideResult)
		...
		-- REJECT UNANCHORED & INVISIBLE: Only paint static, visible map geometry
		if not res.Instance.Anchored or res.Instance.Transparency >= 1 or not res.Instance.CanCollide then
			return
		end
