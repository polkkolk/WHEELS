...
local ds = DataStoreService:GetOrderedDataStore("CoinsLeaderboard")
...
local function refresh()
	...
	for i = 1, 10 do
		...
		local coinsL = get("Coins", i)
		if nameL then nameL.Text = entry and entry.key or "---" end
		if coinsL then coinsL.Text = entry and (tostring(entry.value) .. "$") or "— $" end
	end
end
